...
contract OORTSwapERC20 is IOORTSwapERC20 {
    ...
    string public constant name = 'OORTSwap';
    string public constant symbol = 'OORTSwap LP';
...
contract OORTSwapPair is IOORTSwapPair, OORTSwapERC20 {
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'OORTSwap: FORBIDDEN');
        // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        ...
        feeOn = feeTo != address(0) && IOORTSwapFactory(factory).getPairRate(address(this)) != 9;
        uint _kLast = kLast;
        // gas savings
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    uint denominator = rootK.mul(IOORTSwapFactory(factory).getPairRate(address(this))).add(rootKLast);
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        {// scope for reserve{0,1}Adjusted, avoids stack too deep errors
            uint balance0Adjusted = balance0.mul(1e4).sub(amount0In.mul(IOORTSwapFactory(factory).getPairFees(address(this))));
            uint balance1Adjusted = balance1.mul(1e4).sub(amount1In.mul(IOORTSwapFactory(factory).getPairFees(address(this))));
            require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1e8), 'OORTSwap: K');
        }
...
contract OORTSwapFactory is IOORTSwapFactory {
    ...
    constructor(address _feeToSetter) public {
        ...
        initCodeHash = keccak256(abi.encodePacked(type(OORTSwapPair).creationCode));
    }
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        require(getPair[token0][token1] == address(0), 'OORTSwapFactory: PAIR_EXISTS');
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'OORTSwapFactory: FORBIDDEN');
        feeTo = _feeTo;
    }

    function setFeeToSetter(address _feeToSetter) external {
        require(msg.sender == feeToSetter, 'OORTSwapFactory: FORBIDDEN');
        require(_feeToSetter != address(0), "OORTSwapFactory: FeeToSetter is zero address");
        feeToSetter = _feeToSetter;
    }

    function addPair(address pair) external returns (bool){
        require(msg.sender == feeToSetter, 'OORTSwapFactory: FORBIDDEN');
        require(pair != address(0), 'OORTSwapFactory: pair is the zero address');
    ...
    function setFeeRateNumerator(uint256 _feeRateNumerator) external {
        require(msg.sender == feeToSetter, 'OORTSwapFactory: FORBIDDEN');
        require(_feeRateNumerator <= 30, "OORTSwapFactory: EXCEEDS_FEE_RATE_DENOMINATOR");
    ...
    function setPairFees(address pair, uint256 fee) external {
        require(msg.sender == feeToSetter, 'OORTSwapFactory: FORBIDDEN');
        require(fee <= 30, 'OORTSwapFactory: EXCEEDS_FEE_RATE_DENOMINATOR');
        pairFees[pair] = fee;
    }
    ...
    function setDefaultFeeToRate(uint256 rate) external {
        require(msg.sender == feeToSetter, 'OORTSwapFactory: FORBIDDEN');
        require(rate > 0 && rate <= 10, "OORTSwapFactory: FEE_TO_RATE_OVERFLOW");
    ...
    function getReserves(address tokenA, address tokenB) public view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IOORTSwapPair(pairFor(tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    // given some amount of an asset and pair reserves, returns an equivalent amount of the other asset
    function quote(uint amountA, uint reserveA, uint reserveB) public pure returns (uint amountB) {
        require(amountA > 0, 'OORTSwapFactory: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'OORTSwapFactory: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, address token0, address token1) public view returns (uint amountOut) {
        require(amountIn > 0, 'OORTSwapFactory: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'OORTSwapFactory: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, address token0, address token1) public view returns (uint amountIn) {
        require(amountOut > 0, 'OORTSwapFactory: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'OORTSwapFactory: INSUFFICIENT_LIQUIDITY');
        uint256 fee = getPairFees(pairFor(token0, token1));
    ...
    function getAmountsOut(uint amountIn, address[] memory path) public view returns (uint[] memory amounts) {
        require(path.length >= 2, 'OORTSwapFactory: INVALID_PATH');
